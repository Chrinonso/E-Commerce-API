The Process i took towards building this API
* it can be helpful to you in building a simple API
...
- [] setup / GET Route ( the homepage dummy route)
...
#### Validator Package
it is used in the mongoose to validate (Email and name and others)
...
- [] ignore 'role'
- [] alternative 'admin' setup.. make admin ourselves.

#### Handle Password

- [] UserSchema.pre('save') - hook( this is handled as a Pre-save hook in our Schema)
- this points to User
- bcrypt.genSalt - number of rounds
- bcrypt.hash (do all this in the Models.. set them up as middlewares in your schema)
...
- [] create token, setup cookie and implement
...
#### Authenticate User Setup
 check if token exists and set up functionality to check if token is valid and also authorize based on permissions. (only admin should be able to check all users)
#### Auth User Complete


#### Authorize Permissions Setup
allow only admins get access to all users.  admin
- [] hardcode
...
- [] check for oldPassword and newPassword coming from the req.body(since we are trying to update password)
- [] if one missing 400
...
- [] export (createProduct, getAllProducts,
  getSingleProduct, updateProduct, deleteProduct, uploadImage) functions(require the path module to use this)
...
- [] getAllProducts
- [] getSingleProduct// we first need to get the ID coming from the params and set it into a variable then findOne with it.
- [] updateProduct// first thing is to get the id that you set up in the params, then set it up in a variable and. 
- [] deleteProduct// same functionality and i used remove
...
MV function in express-file upload package helps us to move a file to the specified location.
